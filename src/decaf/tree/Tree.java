...
public abstract class Tree {
    ...
    public static final int DOSUBSTMT = FORLOOP + 1;
    
    public static final int SCOPY = DOSUBSTMT + 1;

    public static final int DCOPY = SCOPY + 1;

    public static final int DOSTMT = DCOPY + 1;

    /**
     * Labelled statements, of type Labelled.
     */
    public static final int LABELLED = DOSTMT + 1;
    ...
    public static final int CASEITEM = CASE + 1;

    public static final int DEFAULTITEM = CASEITEM + 1;

    public static final int SYNCHRONIZED = DEFAULTITEM + 1;
    ...
    public static final int COMPLEX = BOOL + 1;
    public static final int STRING = COMPLEX + 1; 
    ...
    public static class DoStmt extends Tree {
      public List<DoSubStmt> doSubStmts;
        
        public DoStmt(List<DoSubStmt> doSubStmts, DoSubStmt last, Location loc) {
          super(DOSTMT,loc);
          this.doSubStmts = doSubStmts;
          this.doSubStmts.add(last);
        }
        ...
        @Override
          public void printTo(IndentPrintWriter pw) {
              ...
              pw.incIndent();
                ...
                pw.incIndent();
                if (doSubStmts != null) {
                  for (DoSubStmt doSubStmt : doSubStmts)
                    doSubStmt.printTo(pw);
                }
                pw.decIndent();
              pw.decIndent();
          }
    ...
    public static class CaseItem extends Tree {
      public Expr constant;
      public Expr expr;

        public CaseItem(Expr constant, Expr expr, Location loc) {
            super(CASEITEM, loc);
    ...
    public static class DefaultItem extends Tree {
      public Expr expr;

        public DefaultItem(Expr expr, Location loc) {
            super(DEFAULTITEM, loc);
            this.expr = expr;
        }

        @Override
        public void accept(Visitor v) {
            v.visitDefaultItem(this);
        }
    ...
    public static class Case extends Expr {
      ...
      public Case(Expr expr, List<CaseItem> caseItems, DefaultItem defaultItem, Location loc) {
            super(CASE, loc);
            this.expr = expr;
    ...
    public static abstract class Visitor {
        ...
        public void visitCaseItem(CaseItem that) {
            visitTree(that);
        }
        public void visitCase(Case that) {
            visitTree(that);
        }
        public void visitDefaultItem(DefaultItem that) {
            visitTree(that);
        }
