...
DoStmt          :   DO DoBranchList DoSubStmt OD
                    {
                        $$.stmt = new Tree.DoStmt($2.subStmtList,$3.doSubStmt,$1.loc);
                    }
                ;
DoSubStmt       :   Expr ':' Stmt
                    {
...
DoBranchList    :   DoBranchList DoBranch
                    {
                        $$.subStmtList.add($2.doSubStmt);
                    }
                |   /* empty */
                {
                    $$ = new SemValue();
                    $$.subStmtList = new ArrayList<DoSubStmt>();
                }
                ;
...
Expr            :	LValue
                ...
                |   CASE '(' Expr ')' '{' CaseItems DefaultItem '}'
                    {
                        $$.expr = new Tree.Case($3.expr,$6.caseItems,$7.defaultItem,$1.loc);
                    }
                |   CASE '(' Expr ')' '{' DefaultItem '}'
                    {
                        $$.expr = new Tree.Case($3.expr,null,$6.defaultItem,$1.loc);
                    }
                |   DCOPY '(' Expr ')'
                {
...
CaseItem        :   Constant ':' Expr ';'
                    {
                        $$.caseItem = new Tree.CaseItem($1.expr,$3.expr,$1.loc);
                    }
                ;
DefaultItem     :   DEFAULT ':' Expr ';'
                    {
                        $$.defaultItem = new Tree.DefaultItem($3.expr,$1.loc);
                    }
                ;
CaseItems       :   CaseItems CaseItem
                    {
                        $$.caseItems.add($2.caseItem);
                    }
                |   CaseItem
                    {
                        $$.caseItems = new ArrayList<Tree.CaseItem>();
                        $$.caseItems.add($1.caseItem);
                    }
                ;
